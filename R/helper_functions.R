...
process_variables <- function(min_Prevalence = NULL,
  ...
  if (!is.null(Specificity)) {
    ...
    if (Specificity == 0) {
      warning("Specificity was 0, changed to 0.1")
      Specificity = .1
    }
  }
  ...
  if (PPV_NPV == "PPV") {
    ...
    if (is.null(overlay_position_FP) & !is.null(Specificity) & !is.null(limits_Specificity)) {
      ...
      # By default, if Specificity is given, we show Specificity +- 5
      # If you want a range, use limits_Specificity
      cli::cli_alert_info("By default, if Specificity is given, we show Specificity +-5. \nIf you want a range, use limits_Specificity")
    ...
    if (is.null(limits_Specificity)) limits_Specificity <- c(0, 100)
    ...
    if (is.null(overlay_position_FN) & !is.null(Sensitivity) & !is.null(limits_Sensitivity)) {
      ...
      limits_Sensitivity <- c(Sensitivity, 100)
      # warning("* limits_Sensitivity is NULL. Setting limits_Sensitivity = c(", 0, ", ", Sensitivity, ")")
    }
    ...
    # limits_Sensitivity
    if (is.null(limits_Sensitivity)) limits_Sensitivity <- c(0, 100)
  ...
  # if (exists("min_FP") & exists("max_FP")) {
  #   
  #   if(min_FP == 100 & max_FP == 100) min_FP = 1
  # }
  ...
  if (overlay == "area" | overlay == "line") {
    ...
    if (overlay == "line" & !is.null(overlay_labels) & length(overlay_labels) != 1) {
      if (length(overlay_prevalence_1) != length(overlay_labels)) stop("* overlay_labels needs 0, 1 or the same number of values that overlay_prevalence_1 (", length(overlay_prevalence_1), "). Now it has ", length(overlay_labels))
    }
  ...
  # General CHECKS
  if (!is.null(Sensitivity) & overlay != c("line")) {
    if (Sensitivity > 100 | Sensitivity < 0) stop("* Sensitivity should be a value 0-100")
  }
  if (!is.null(Specificity) & overlay != c("line")) {
...
.createPPVmatrix <-
  function(min_Prevalence = 1,
    ...
    PPV_melted <- PPV_melted %>%
      ...
      dplyr::mutate(
        prevalence_pct = prevalence_1 / prevalence_2,
...
.get_point_ppv_npv <- function(PPV_melted,
                               PPV_NPV = "PPV",
                               Language,
                               Sensitivity,
                               Specificity,
  ...
  if (PPV_NPV == "NPV") {
    ...
    DEBUG_MESSAGE <- paste0(
      "TRUE_positives: ", TRUE_positives, " | Specificity: ", Specificity, " | overlay_position_FN: ", overlay_position_FN, " | overlay_prevalence_1: ", overlay_prevalence_1, " | overlay_prevalence_2: ", overlay_prevalence_2, "\n",
    ...
    DEBUG_MESSAGE <- paste0(
      "Sensitivity: ", Sensitivity, " | overlay_position_FP: ", overlay_position_FP, " | overlay_prevalence_1: ", overlay_prevalence_1, " | overlay_prevalence_2: ", overlay_prevalence_2, "\n",
